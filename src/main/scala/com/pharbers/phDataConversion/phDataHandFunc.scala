...
object phDataHandFunc {
    ...
    val setHospCol: UserDefinedFunction = udf{
        (_id: String,title: String, `type`: String, level: String, character: String, addressID: String) =>
            hospData(_id, `type`, title, level, character, addressID).toString
//        _id: String => hospData(_id)
    ...
    def string2HospData(s: String): hospData = {
        val atts = s.split(",")
        hospData(atts(0), atts(1), atts(2), atts(3),atts(4),atts(5))
    }
}
