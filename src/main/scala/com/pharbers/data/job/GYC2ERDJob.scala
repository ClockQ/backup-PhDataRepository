...
case class GYC2ERDJob(args: Map[String, String])(implicit any: Any = null) extends sequenceJobWithMap {
    ...
    val gyc_file: String = args("gyc_file")
    ...
    override def perform(pr: pActionArgs = MapArgs(Map())): pActionArgs = {
        ...
        val gycDF = CSV2DF(gyc_file)
        val gycDFCount: Long = gycDF.count()
        ...
        val gycProdCount: Long = gycProd.count()
        val gycHosp = gycResult("hospDIS")
        val gycHospCount: Long = gycHosp.count()
        val gycPha = gycResult("phaDIS")
